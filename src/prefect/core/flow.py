...
class Flow:
    ...
    Args:
        ...
        -  terminal_state_handler (Callable, optional): A state handler that mutates the final state
            of the flow, with signature `state_handler(state: State, key_states: Set[State],
            return_states: Dict[Task, State], terminal_states: Set[State],) -> State`.
            `state` is the current state of the Flow. `key_states` are the states that will determine
            the success / failure of the flow run. `return_states` contains states for Flow Tasks.
            `terminal_states` are the states of terminal tasks for this Flow.
    """
